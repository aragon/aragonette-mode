...
export async function fetchIpfsAsJson(ipfsUri: string) {
  const res = await fetchRawIpfs(ipfsUri);
  return await res.json();
}

export async function fetchIpfsAsText(ipfsUri: string) {
  const res = await fetchRawIpfs(ipfsUri);
  return await res.text();
}

export async function fetchIpfsAsBlob(ipfsUri: string) {
  const res = await fetchRawIpfs(ipfsUri);
